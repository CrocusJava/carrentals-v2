...
public class OrderTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		formatter.format("<option value=\"%d\">%s,%s</option>",  vpget.getIdPlace(), vpget.getCity(), vpget.getDescription());
		formatter.format("</select></td></tr><tr><td>");	
		formatter.format(placeReturn+"</td><td><select name=\"idplacereturn\">");
		for (Place place : value) {
			formatter.format("<option value=\"%d\">%s,%s</option>", place.getIdPlace(), place.getCity(), place.getDescription());
		}
		formatter.format("</select></td></tr><tr><td>");
